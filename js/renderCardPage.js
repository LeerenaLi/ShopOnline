...
import {addProductData} from './serviseStorage.js';


export const titleCard = document.querySelector('.card__title');
...
export const createBook = (price, id) => {
    ...
    addCartBtn.classList.add('book__btn', 'btn');
    addCartBtn.id = 'add';
    addCartBtn.dataset.goodid = `${id}`;
    addCartBtn.textContent = 'Добавить в корзину';

    const favoriteBtn = document.createElement('button');
    favoriteBtn.classList.add('book__favorite');
    ...
    buttonsWrapper.append(addCartBtn, favoriteBtn);
    ...
    return {bookWrapper, addCartBtn, favoriteBtn};
};

const addRecommendGoods = async () => {
...
export const cardPageControl = async () => {
    ...
    createProductPhoto(title, discount, imgUrl);

    cardDescription.textContent = description;

    const {addCartBtn, favoriteBtn} = createBook(price, id);
